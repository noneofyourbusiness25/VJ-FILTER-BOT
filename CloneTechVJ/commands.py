# Don't Remove Credit @R_Bots_Updates
...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            files1 = files_[0]
            title = '@R_Bots_Updates  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))
            size=get_size(files1.file_size)
            f_caption=files1.caption
            if f_caption is None:
                f_caption = f"@R_Bots_Updates  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files1.file_name.split()))}"
            if cd["update_channel_link"] != None:
                up = cd["update_channel_link"]
                button = [[
...
async def settings(client, message):
    ...
    try:
        ...
        link = 'https://t.me/R_Bots_Updates'
